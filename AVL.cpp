...
public:
    ...
    void smallTurnLeft(Node* x, Node* y)
    {
        if (x->parent)
            x->parent->right = y;
        y->parent = x->parent;
    ...
    void smallTurnRight(Node* x, Node* y)
    {
        if (x->parent)
            x->parent->left = y;
        y->parent = x->parent;
    ...
    Node* findTheClosest(const string& name)
    {
        Node* privious = root;;
        for (auto node = root; node;)
        {
    ...
    Node* addNode(const string& name)
    {
        ...
        Node* closest = findTheClosest(name);
...
{
    Tree* result = new Tree();
    assert(result->RmNode("GGT") == nullptr);
    auto abc = result->addNode("Abc");
    auto def = result->addNode("Def");
    auto ghi = result->addNode("GHi");
    auto aac = result->addNode("AAC");
    ...
    assert(result->RmNode("GHi") == ghi);
    assert(ghi->getParent() == nullptr);
    assert(ghi->getLeft() == nullptr);
    assert(ghi->getRight() == nullptr);
    assert(gqa->getLeft() == nullptr);
    assert(result->RmNode("GGT") == nullptr);
    assert(result->RmNode("GQA") == gqa);
...
int main()
{
    std::cerr << "Test" << std::endl;   //no mistakes
    testAddNode();
    testRM();
    testiterator();
    return 0;
